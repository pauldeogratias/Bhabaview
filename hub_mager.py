...
"""
    ...
    def view_file(self):
        ...
        item = selection[0]
        try:
            data_str = self.file_tree.set(item, 'full_data')
            if not data_str:
                messagebox.showwarning("Warning", "Invalid file selection")
                return
                
            data = json.loads(data_str)
            
            if data['type'] == 'file':
                self.update_status(f"Loading file: {data['name']}")
                Thread(target=self._fetch_and_show_file, args=(data,), daemon=True).start()
            else:
                messagebox.showinfo("Info", "Selected item is a folder, not a file")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to read file data: {str(e)}")
    ...
    def _fetch_and_show_file(self, file_data):
        ...
        try:
            ...
            self.root.after(0, lambda: self.update_status(f"Downloading {file_data['name']}..."))
            response = requests.get(url, headers=headers)
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    # If it's a binary file, show a message instead
                    content = f"[Binary file: {file_data['name']}]\n\nThis file contains binary data and cannot be displayed as text.\nFile size: {self.format_size(file_data['size'])}"
                
                self.root.after(0, self._show_file_content, file_data['name'], content, file_data['path'])
            else:
                self.root.after(0, lambda: messagebox.showerror("Error", f"Failed to fetch file: {response.status_code}"))
    ...
    def _show_file_content(self, filename, content, filepath):
        """Show file content in a new window"""
        viewer = tk.Toplevel(self.root)
        viewer.title(f"File Viewer: {filepath}")
        viewer.geometry("900x700")
        viewer.configure(bg='#0f1419')
        
        # Header with file info
        header_frame = ttk.Frame(viewer, style='Dark.TFrame')
        header_frame.pack(fill='x', padx=10, pady=5)
        
        ttk.Label(header_frame, text=f"📄 {filename}", style='Heading.TLabel').pack(side='left')
        ttk.Label(header_frame, text=f"Path: {filepath}", style='Dark.TLabel').pack(side='right')
        
        # Create text widget with scrollbars
        text_frame = ttk.Frame(viewer, style='Dark.TFrame')
        text_frame.pack(fill='both', expand=True, padx=10, pady=5)
        
        text_widget = tk.Text(text_frame, bg='#1e2328', fg='#e6e6e6', font=('Consolas', 11), wrap='none')
        ...
        # Bottom buttons
        button_frame = ttk.Frame(viewer, style='Dark.TFrame')
        button_frame.pack(fill='x', padx=10, pady=5)
        
        ttk.Button(button_frame, text="Close", command=viewer.destroy, style='Dark.TButton').pack(side='right', padx=5)
        ttk.Button(button_frame, text="Copy Content", 
                  command=lambda: self._copy_to_clipboard(content), style='Dark.TButton').pack(side='right', padx=5)
